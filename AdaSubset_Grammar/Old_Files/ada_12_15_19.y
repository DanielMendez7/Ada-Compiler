/***************************** I think I finished exceptions, now moving onto fixing my full else if statements 12_15_19  *********************/
%{
int yylex();
int yyerror(char*);
#include <stdio.h>
...
main_specification      : PROCEDURE ID                                    {
                                                                           ...
                                                                           excepPushList(stackOfLLExcep);  //TODO: <--- NEW PUSH (Main Patch for exceptions)
...
procedure_body          : procedure_body_prime IS                         
                          ...
                          exception_part END ';'                          {
                                                                           ...
                                                                           /***********/
                                                                           //TODO: PART 5 Get actuals addr and copy formal parameter addres to a
                                                                           ...
                                                                           while (parameterList != NULL) {
                                                                               ...
                                                                               //TODO: Put actual values list into the copy list because actual values list will get lost after popping the procedure
                                                                           ...
                                                                           excepPopList(stackOfLLExcep); //TODO: <--- NEW Pop procedures exception toPatch linked list
...
procedure_body_prime    : procedure_specification formal_parameter_part   {
                                                                           ...
                                                                           //Updated version accounts for duplicates and connects precedure node to the formal parameters in the next binary tree
                                                                            ...
                                                                            /***********/
                                                                            //TODO: PART 5 Procedures with parameters
...
procedure_specification : PROCEDURE ID
                                          {
									          ...
									          push(stackOfTrees, $2);  //Push new binaryTree onto stack
									          pushOffSet(stackOffSet); //Push new offset onto stack because of new procedure
                                              excepPushList(stackOfLLExcep);  //TODO: <--- NEW PUSH (Procedures Patch for exceptions)
...
proc_read_write_arrays_assign : ID optional_parameters optional_assign
                                                               {
                                                                  ...
                                                                  //KIND IS PROCEDURE(enum 2): a procedure so calculate static, dynamic, next base, and return address
                                                                      ...
                                                                      /***************************/
                                                                      //TODO: Part 5 Fix in/in out parameters  Fix procedure call with parameters
                                                                      ...
                                                                      //TODO: This is for exceptions in ada. A jump to jump to exception table if r1 != 0
...
statement_sequence_prime: NULLWORD
                        ...
                        //How is this running without a ; meta symbol here?
                        ;
...
case_start      : CASE             {
                                      //pushList(stackOfLL);// For first when statement patch
                                      //$$ = globalRecord.pc; 
                                   } //for When statement Pc jumps; }
                ;


case_expression : expression
                                   {
                                       //fprintf(output, "%d: pc := ? if not contents b, %d\n", globalRecord.pc++, $1->offset);
                                       ...
                                       //putInToList(stackOfLL, globalRecord.pc + 1); // Need to patch the first when statement
                                       //$$ = globalRecord.pc - 1;
                                   }
                ;
...
when_expression  : expression
                                     {
                                        ...
                                        //printAllNodes(stackOfLL[patchTopIndex].leaveAddressPtr); //NEW

                                        //patchList = appendToList(patchList, globalRecord.pc, globalRecord.pc + 2);
                                        //globalRecord.pc++;
                                        putInToList(stackOfLL, globalRecord.pc++);
...
int main()
{
    ...
    add(stackOfTrees, 8, "contraint_error");  //Predeclared
    ...
    Node* constraintErrorNode = localSearch(stackOfTrees[top].rootPointer, "contraint_error");  
...
/*
//Accounts for case statements in Ada program
case_statement : case_start case_expression IS case_list ENDCASE
               ;
...
case_handler : WHEN case_choice ARROW sequence_of_statements
;
...
/**********************************************/
//Accounts for case statements in Ada program
//TRY THIS ONE WORKS WITHOUT OR BARS 
/*
case_statement  : case_start case_expression IS case_list ENDCASE
                                  {
...
case_handler    : when_start  when_expression  ARROW sequence_of_statements 
                                    {
                                      ListNodePtr tempHead = stackOfLL[patchTopIndex].leaveAddressPtr;
                                      while (tempHead != NULL){
                                          patchList = appendToList(patchList, tempHead->patchLine, globalRecord.pc+1);
...
when_expression          : when_expression_sequence
                         ;
 

when_expression_sequence : expression '|' when_expression_sequence
                                     {

                                        //fprintf(output, "%d\n", $1->registerNumber);
                                        
                                        //fprintf(output, "%d: r%d := r%d = r%d\n",
                                        //globalRecord.pc++, globalRecord.registerNumber, $3->registerNumber-1, stackOfReg[regTopIndex].registerNumber);
...
when_expression          : when_expression_sequence
                                    {
                                        ...
                                        //NumListNodePtr traverseList = $1;
                                        //if (sizeOfList(traverseList) > 1) {
                                        //    while (traverseList != NULL) {  //stops at the very last node
                                        //        fprintf(output, "%d: pc := ?\n", globalRecord.pc);
                                        //        traverseList = traverseList->next;
                                        //    }
                                        // }
...
when_expression_sequence : expression '|' when_expression_sequence
                                     {
                                        ...
                                        //fprintf(output, "%d: pc := ?\n", globalRecord.pc);
                                         ...
                                         NumListNodePtr wholeList = addNumToList(initNumNode($1->registerNumber), $3);
                                         $$ = wholeList;
                         ...
                         | expression
                                     {
                                        ...
                                        NumListNodePtr temp = initNumNode($1->registerNumber);
                                        $$ = temp;
...
/*
case_handler    : when_start  when_expression_list  ARROW sequence_of_statements
                                {
                                   ListNodePtr tempHead = stackOfLL[patchTopIndex].leaveAddressPtr;
                                   if (tempHead != NULL){
                                       patchList = appendToList(patchList, tempHead->patchLine, $1);
                                   }
...
when_expression_list   : when_expression '|' when_expression_list
                       | when_expression
                       ;
...
when_start      : WHEN  { putInToList(stackOfLL, globalRecord.pc); } //pc jump to end case
                ;
