/***************************** Something is screwed up with my records linked list data structure *********************/
...
%type <ptrToRecordNode>  expression_list   optional_parameters    record_object   component_list //If you get a segfault its probably from this data structure
...
component_list          : record_object ';' component_list { $$ = addToRecordList($1, $3); } // can put record into symbol table but not its individual components
                        | record_object ';'                { $$ = $1; }
                        ;
...
record_object           : identifier_list ':' type_name
                                                               {
                                                                   ...
                                                                   RecordNodePtr recordListHead = NULL;
                                                                   ...
                                                                   RecordNodePtr newHead;

                                                                   recordptr currentRecord;
                                                                   ...
                                                                   //Ex. a, b, c, : integer
                                                                       ...
                                                                       while (traverseList != NULL) {
                                                                           ...
                                                                           if (strcmp(varNodeType->symbol, "integer") == 0 ||
                                                                               ...
                                                                               //Create the record with all the information needed
                                                                               //currentRecord->variableName = (char*) malloc(sizeof(traverseList->name) + 1); //Name of variable, ex. a
                                                                               strcpy(currentRecord->variableName, traverseList->name);
                                                                               ...
                                                                               //  currentRecord->offset = returnOffSet(stackOffSet); //Set outcome->offset equal to the current offset
                                                                               
                                                                               //Increment globalOfsset by first->parent_type->offset
                                                                               //  raiseOffSet(stackOffSet, currentRecord->parent_type->offset); //integer and boolean offset is 1
                                                                               ...
                                                                               currentRecord->memSpace = currentRecord->parent_type->offset;
                                                                           }

                                                                           //TODO: arrays declared inside of records //FOR PART 2aii

                                                                           //TODO: records declared inside of records //FOR PART 2bii
                                                                           
                                                                           //Create the linked list of record node pointers
                                                                           //printf(" is current_record NULL? %s", currentRecord == NULL ? "true\n" : "false\n");
                                                                           //printf(" outside if statement: %s\n ", currentRecord->variableName);
                                                                           newHead = createRecordNode(currentRecord);
                                                                           //printf("******\n");
                                                                           //printSubVariables(newHead);
                                                                           
                                                                           recordListHead = addToRecordList(newHead, recordListHead);
                                                                     ...
                                                                     if (localSearch(stackOfTrees[top].rootPointer, traverseList->name) != NULL) {
                                                                         ...
                                                                         if (varNodeType != NULL) {
                                                                             ...
                                                                             //y will NOT have a component type
                                                                                 ...
                                                                                 //printf("**********%s IS A RECORD*********\n", first->symbol);
                                                                                 //TODO: Do stuff with offsets here
                                                                                 //RecordNodePtr traverseVariables = first->listOfVariables;
                                                                                 //while (traverseVariables != NULL) {
                                                                                 //traverseVariables->record->offset = returnOffSet(stackOffSet);
                                                                                 //raiseOffSet(stackOffSet, traverseVariables->record->memSpace); // Increment offset by the amount taken up by this variable

                                                                                 //traverseVariables = traverseVariables->next;
                                                                                 //}
                        ...
                        | primary '.' ID
                                                     {
                                                         ...
                                                         RecordNodePtr traverseList = nodeInTree->listOfVariables;

                                                         printf("Searching for: %s\n", $3);
                                                         
                                                         while( (traverseList != NULL) &&  !(strcmp(traverseList->record->variableName, $3) == 0) ) {
                                                             subVariableOffSet = subVariableOffSet + traverseList->record->memSpace;
                                                             printf("In primary production %s\n", traverseList->record->variableName); //TODO: Fix this error!
                                                         ...
                                                         if (traverseList != NULL) {
                                                            //Add on the base offset of variable of type Ada record
                                                             ...
                                                             subVariableInfo->kind = traverseList->record->kind;
